...
interface SmartStaffSearchProps {
  onStaffSelect: (staff: Staff) => void;
  placeholder?: string;
  className?: string;
}

export default function SmartStaffSearch({ 
  onStaffSelect, 
  placeholder = "Search by name or staff ID...",
  className = ""
}: SmartStaffSearchProps) {
              ...
              <div className="flex justify-between items-start">
                <div className="flex-1">
                  <div className={`font-medium ${
                    ...
                    {highlightMatch(staff.fullName, searchTerm)}
                  </div>
                  <div className={`text-sm ${
                    index === selectedIndex ? 'text-white' : 'text-gray-500 dark:text-gray-400'
                  }`}>
                    ID: {staff.staffId} • {staff.department} • {staff.position}
                  </div>
                </div>
              </div>
            </div>
