...
export default function DeanOfStudentsStudents() {
    ...
    if (authLoading || loading) {
        return <div className="text-center p-12 text-kmuGreen uppercase font-bold tracking-widest">Loading behavioral registry...</div>;
    }
            ...
            <div className="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
                <div className="animate-in fade-in duration-500 space-y-6">
                    ...
                    <div className="flex flex-col md:flex-row justify-between items-start md:items-center bg-white dark:bg-gray-900 p-6 rounded-xl shadow-sm border border-gray-200 dark:border-gray-800 gap-4">
                        <div>
                            <h1 className="text-2xl font-bold uppercase tracking-tight">Student behavioral Registry</h1>
                            <p className="text-sm text-gray-500 font-medium mt-1 uppercase tracking-wider">Comprehensive student directory</p>
                        </div>
                    </div>
                    ...
                    <div className="bg-white dark:bg-gray-900 rounded-xl shadow-sm border border-gray-200 dark:border-gray-800 overflow-hidden">
                        ...
                        <div className="p-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 font-sans">
                            {filteredStudents.map((s, i) => (
                                <div
                                    key={s._id || i}
                                    className="bg-gray-50 dark:bg-gray-800/50 p-4 rounded-xl border border-gray-100 dark:border-gray-700 hover:border-emerald-500/50 hover:shadow-md transition-all cursor-pointer group"
                                    onClick={() => router.push(`/students/${s._id}`)}
                                >
                                    <div className="flex items-center gap-4 mb-3">
                                        <div className="w-10 h-10 rounded-lg bg-emerald-100 dark:bg-emerald-950/30 flex items-center justify-center text-emerald-600 font-bold group-hover:scale-110 transition-transform text-lg">
                                            {s.fullName?.charAt(0)}
                                        </div>
                                        <div>
                                            <div className="font-bold text-xs uppercase dark:text-gray-100 group-hover:text-emerald-600 transition-colors line-clamp-1 leading-tight">{s.fullName}</div>
                                            <div className="text-[10px] text-gray-400 font-bold uppercase tracking-tight mt-0.5">{s.studentId}</div>
                                        </div>
                                    </div>
                                    <div className="pt-3 border-t border-gray-100 dark:border-gray-700 flex justify-between items-end">
                                        <div className="text-[9px] font-black text-gray-500 uppercase tracking-widest truncate max-w-[120px]">{s.program}</div>
                                        <span className="text-[8px] bg-emerald-100 dark:bg-emerald-900/30 text-emerald-600 px-1.5 py-0.5 rounded font-black uppercase tracking-tighter">Verified</span>
                                    </div>
                                </div>
                            ))}
